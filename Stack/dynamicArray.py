...
#---------------------------------------------------------------#    
    ...
    def bubblesort(self):
        """
        Function designed to implement the bubble sort algorithm on the
        dynamic array in ascending order
        """

        # checking if the list is empty
        if self.is_empty():
            raise Dynamic_Array_Exception("EMPTY LIST")

        # starting a counter at end of array iterating backwards to the front
        for x in range((self._size - 1), 0, -1):

            # starting a counter at 0 iterating until it meets outer loop counter
            for y in range(x):

                # checking if current index is greater than the next index
                if self._data.get_at_index(y) > self._data.get_at_index(y + 1):

                    # switching values if so
                    temp = self._data.get_at_index(y)
                    self._data.set_at_index(y, self._data.get_at_index(y + 1))
                    self._data.set_at_index(y + 1, temp)
    ...
    def mergesort(self):
        ...
        if self._size > 1:
            ...
            # making two lists to hold each half of given list
            left = Dynamic_Array()
            leftSize = self._size / 2
            right = Dynamic_Array()
            rightSize = self._size - leftSize

            for x in range(int(leftSize)):
                left.append(self._data.get_at_index(x))


            for x in range(int(leftSize), self._size, 1):
                right.append(self._data.get_at_index(x))

            right.mergesort()
            left.mergesort()
            
            newArr = Dynamic_Array()
            #newArr = self.merge(left, right)
    ...
    #def merge(self, left: Dynamic_Array, right: Dynamic_Array) -> Dynamic_Array:
        # need to merge the two dynamic array objects
...
if __name__ == '__main__':
    array = Dynamic_Array()
    rangeVal = random.randint(5, 100)

    for i in range(rangeVal):
        num = random.randint(10, 332)
        array.append(num)
    ...
    print("Min value: ", array.get_min())
    print("Max value: ", array.get_max())
